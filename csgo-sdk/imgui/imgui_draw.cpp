...
void normalize(float& val) {
	while (val > 1.f) val -= 1.f;
	while (val < 0.f) val += 1.f;
}
...
void ImDrawList::AddLineColoured(const ImVec2& a, const ImVec2& b, ImU32 col, float thickness /*= 1*/) {
	ImVec4 aaa = ImGui::ColorConvertU32ToFloat4(col);
	aaa.w = min(aaa.w+ 0.4f, 1.f);
	col = ImGui::ColorConvertFloat4ToU32(aaa);
	float hashs = a.x + a.y + b.x + b.y;
	static std::map<float, float> mapp;
	bool first = mapp.find(hashs) == mapp.end();
	if (first) {
		mapp[hashs] = GLOBAL_H_COLOR;
		GLOBAL_H_COLOR += (1.f / 6.f); normalize(GLOBAL_H_COLOR);
	}
	else mapp[hashs] += 0.9985f; normalize(mapp[hashs]);
	float next = mapp[hashs] + (1.f / 6.f); normalize(next);
	ImU32 colleft = ImGui::GetColorU32(ImColor::HSV(next, 1.f, 1.f, (col >> 24) * (1.f / 255.f)));
	ImU32 colright = ImGui::GetColorU32(ImColor::HSV(mapp[hashs], 1.f, 1.f, (col >> 24) * (1.f / 255.f)));
	AddRectFilledMultiColor(a, b + ImVec2(0, thickness), colleft, colright, colright, colleft);
}

void ImDrawList::AddLine(const ImVec2& a, const ImVec2& b, ImU32 col, float thickness) {
	if (GLOBAL_RAINBOW)
		return AddLineColoured(a, b, col, thickness);
...
void ImDrawList::AddRect(const ImVec2& a, const ImVec2& b, ImU32 col, float rounding, int rounding_corners, float thickness) {
	if (GLOBAL_RAINBOW) {
		...
		ImU32 colstart = ImGui::GetColorU32(ImColor::HSV(mapp[hashs], 1.f, 1.f, (col >> 24) / 255.f));
		float next = mapp[hashs] + (1.f / 4.f); normalize(next);
		ImU32 colnext = ImGui::GetColorU32(ImColor::HSV(next, 1.f, 1.f, (col >> 24) * (1.f / 255.f)));
		AddRectFilledMultiColor(a, (ImVec2(b.x, a.y) + ImVec2(0.f, thickness)), colstart, colnext, colnext, colstart);
		colstart = colnext;
		next = next + (1.f / 4.f); normalize(next);
		colnext = ImGui::GetColorU32(ImColor::HSV(next, 1.f, 1.f, (col >> 24) * (1.f / 255.f)));
		AddRectFilledMultiColor(ImVec2(b.x, a.y), b + ImVec2(thickness, 0), colstart, colstart, colnext, colnext);
		colstart = colnext;
		next = next + (1.f / 4.f); normalize(next);
		colnext = ImGui::GetColorU32(ImColor::HSV(next, 1.f, 1.f, (col >> 24) * (1.f / 255.f)));
		AddRectFilledMultiColor(b, ImVec2(a.x, b.y) - ImVec2(0, thickness), colstart, colnext, colnext, colstart);
		colstart = colnext;
		next = next + (1.f / 4.f); normalize(next);
		colnext = ImGui::GetColorU32(ImColor::HSV(next, 1.f, 1.f, (col >> 24) * (1.f / 255.f)));
		AddRectFilledMultiColor(ImVec2(a.x, b.y), a - ImVec2(thickness, 0), colstart, colstart, colnext, colnext);
		return;
	}
