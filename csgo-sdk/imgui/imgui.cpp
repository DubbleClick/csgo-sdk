...
void ImGui::RenderTextTitle(const ImVec2& pos_min, const ImVec2& pos_max, const char* text, const char* text_end, const ImVec2* text_size_if_known, ImGuiAlign align, const ImVec2* clip_min, const ImVec2* clip_max) {
	if (!strcmp(text, "Debug"))
		return;

	//if (!GLOBAL_RAINBOW)
	//	return RenderTextClipped(pos_min, pos_max, text, text_end, text_size_if_known, align, clip_min, clip_max);
	...
	for (int i = 0; i < text_len; i++) {
		static float h = 0.1f, s = 1.f, v = 1.f, a = 1.f;
		h += (1.f/(float)text_len) - 0.00005f;
		if (h > 1.f)
			h -= 1.f;
		ImU32 color = GetColorU32(ImColor::HSV(h, s, v, a));
		...
		if (need_clipping) {
			...
			window->DrawList->AddText(g.Font, g.FontSize, pos, color, &text[i], &text[i + 1], 0.0f, &fine_clip_rect);
		}
		else {
			window->DrawList->AddText(g.Font, g.FontSize, pos, color, &text[i], &text[i + 1], 0.0f, NULL);
		}

		pos.x += CalcTextSize(lol).x;
	}
}
	...
	if (first_begin_of_the_frame) {
		...
		if (!(flags & ImGuiWindowFlags_NoTitleBar)) {
			if (p_open != NULL) {
				...
				if (strcmp(name, "Retaliate")) //no close button for hack main window
			...
			RenderTextTitle(text_min, text_max, name, NULL, &text_size, style.WindowTitleAlign, NULL, &clip_max);
		}
...
bool ImGui::Checkbox(const char* label, bool* v) {
	...
	if (*v) {
		...
		const ImVec2 points[3] = { ImVec2(check_mark_bb.GetCenter().x - check_mark_bb.GetWidth() / 2, check_mark_bb.GetCenter().y), ImVec2(check_mark_bb.GetCenter().x, check_mark_bb.GetCenter().y + check_mark_bb.GetHeight() / 2), ImVec2(check_mark_bb.GetTR()) };
		window->DrawList->AddPolyline(points, 3, GetColorU32(ImGuiCol_CheckMark), false, 3.0f, true);
	}
